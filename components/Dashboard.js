...
export default function Dashboard() {
  ...
  return (
    <div className='flex flex-col flex-1 gap-8 sm:gap-10 md:gap-16'>
      <div className='grid grid-cols-3 bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 p-4 gap-4 rounded-lg'>
        {Object.keys(statuses).map((status, statusIndex) => (
          <div key={statusIndex} className='flex flex-col gap-1 sm:gap-2'>
            <p className='font-medium capitalize text-xs sm:text-sm truncate'>{status.replaceAll('_', ' ')}</p>
            <p className={'text-base sm:text-lg truncate ' + orbitron.className}>
      ...
      <div className='flex items-stretch flex-wrap gap-4'>
        {Object.keys(tracks).map((track, trackIndex) => (
          <button
            ...
            className='p-4 px-5 rounded-2xl grayShadow duration-200 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 text-center flex flex-col items-center gap-2 flex-1'
            key={trackIndex}
          >
            <p className='text-4xl sm:text-5xl md:text-6xl'>{tracks[track]}</p>
            <p className={'text-gray-500 dark:text-gray-400 text-xs sm:text-sm md:text-base ' + orbitron.className}>{track}</p>
          </button>
        ))}
      </div>
